...
print("  1. d4_ground_truth.csv   -> original file, unmodified")
print("  2. d4_annotator1.csv     -> fully filled in by Annotator 1")
print("  3. d4_annotator2.csv     -> fully filled in by Annotator 2")
...
print("  - Each label is EXACTLY one of: anger, fear, happiness, hatred, sadness, wonder")
...
# auto-renamed them (e.g. "d4_annotator1 (1).csv") due to a naming clash
expected_files = ['d4_ground_truth.csv', 'd4_annotator1.csv', 'd4_annotator2.csv']
uploaded_names = list(uploaded.keys())

def find_uploaded_match(expected_name, uploaded_names):
    base = expected_name.replace('.csv', '')
    # Matches "d4_annotator1.csv", "d4_annotator1 (1).csv", "d4_annotator1(2).csv", etc.
...
gt_df = pd.read_csv(file_map['d4_ground_truth.csv'], encoding='utf-8-sig')
a1_df = pd.read_csv(file_map['d4_annotator1.csv'], encoding='utf-8-sig')
a2_df = pd.read_csv(file_map['d4_annotator2.csv'], encoding='utf-8-sig')
...
ACCEPTED_LABELS = ['anger', 'fear', 'happiness', 'hatred', 'sadness', 'wonder']
...
# 'consensus' is left blank here on purpose: it will be filled in manually
# after Annotator 1 and Annotator 2 sit together, discuss every text where
# their labels disagreed, revisit the annotation guidelines, and agree on
# one final label per text. This reconciled column — not annotator1 or
# annotator2 alone — is what gets compared against ground_truth for the
# final accuracy figure reported in Section 3.1.2.
summary_df['consensus'] = ''

summary_df.to_csv('d4_combined_annotations.csv', index=False, encoding='utf-8-sig')
files.download('d4_combined_annotations.csv')