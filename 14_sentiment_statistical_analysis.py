...
BASE_FILES_MODELS = {
    'Claude_sentiment_results': 'Claude',
    'DeepSeek_sentiment_results': 'DeepSeek',
    'Gemini_sentiment_results': 'Gemini',
    'GPT4o_sentiment_results': 'GPT4o'
}
        ...
        if model_name:
            try:
                ...
                if merged_df is None:
                    ...
                    merged_df = merged_df.rename(columns={'predicted_sentiment': model_name})
                else:
                    merged_df[model_name] = temp_df['predicted_sentiment']
...
def bootstrap_metric(y_true, y_pred, metric_func, metric_name, n_bootstrap=1000, confidence_level=0.95, **kwargs):
    ...
    df_temp = pd.DataFrame({'sentiment': y_true, 'pred': y_pred})
    df_clean = clean_sentiment_data(df_temp, ['sentiment', 'pred'])
    ...
    for _ in range(n_bootstrap):
        ...
        bootstrap_true = df_clean['sentiment'].iloc[bootstrap_indices]
...
def mcnemar_test(y_true, y_pred1, y_pred2, model1_name, model2_name):
    ...
    df_temp = pd.DataFrame({'sentiment': y_true, 'pred1': y_pred1, 'pred2': y_pred2})
    df_clean = clean_sentiment_data(df_temp, ['sentiment', 'pred1', 'pred2'])
    ...
    correct1 = (df_clean['sentiment'] == df_clean['pred1']).astype(int)
    correct2 = (df_clean['sentiment'] == df_clean['pred2']).astype(int)
...
def calculate_model_metrics(df, model_columns):
    ...
    # Clean the dataframe based on the 'sentiment' column
    df_sentiment_clean = clean_sentiment_data(df, ['sentiment'])

    if len(df_sentiment_clean) == 0:
        print("⚠️  No valid samples found for basic metrics calculation.")
        return {}

    for model in model_columns:
        # For each model, filter for valid sentiment and valid model predictions
        df_clean_model = clean_sentiment_data(df_sentiment_clean, [model])
...
def perform_statistical_analysis(df):
    ...
    for model in model_columns:
        ...
        # Ensure 'sentiment' column exists before calling bootstrap functions
        if 'sentiment' in df.columns:
            acc_bootstrap = bootstrap_accuracy(df['sentiment'], df[model])
            f1_bootstrap = bootstrap_f1_score(df['sentiment'], df[model])
            ...
            print(f"⚠️  'sentiment' column not found in main DataFrame. Skipping bootstrap for {model}.")
    ...
    # Ensure 'sentiment' column exists before performing McNemar's test
    if 'sentiment' in df.columns:
        for model1, model2 in combinations(model_columns, 2):
            print(f"\n{model1} vs {model2}:")
            mcnemar_result = mcnemar_test(df['sentiment'], df[model1], df[model2], model1, model2)
         ...
         print("\n⚠️  'sentiment' column not found in main DataFrame. Skipping McNemar's test.")
...
def create_visualizations(df, results):
    ...
    df_viz = clean_sentiment_data(df, ['sentiment'] + model_columns)
    if len(df_viz) == 0:
    ...
    if 'sentiment' in df_viz.columns: # Check if 'sentiment' column exists for per-class accuracy
    ...
    if 'sentiment' in df_viz.columns:
        ax = axes[plot_idx]
        sentiment_classes = sorted(df_viz['sentiment'].unique())
        class_accuracies = {}
        ...
        for model in model_columns:
            ...
            for sentiment in sentiment_classes:
                ...
                if mask.sum() > 0:
                    correct = (df_viz.loc[mask, 'sentiment'] == df_viz.loc[mask, model]).sum()
                    total = mask.sum()
    ...
    plt.tight_layout()
    plt.savefig('sentiment_analysis_statistical_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def save_results_to_file(results, df):
    ...
    with open('statistical_analysis_report.txt', 'w', encoding='utf-8') as f:
        f.write("SENTIMENT ANALYSIS STATISTICAL TESTING REPORT\n")
        f.write("=" * 60 + "\n\n")
        ...
        valid_df = clean_sentiment_data(df, ['sentiment'])
        if 'sentiment' in valid_df.columns:
             f.write(f"Total valid samples for analysis: {len(valid_df)}\n")

             if len(valid_df) > 0:
                f.write(f"Sentiment classes found: {valid_df['sentiment'].unique()}\n")
                f.write(f"Class distribution:\n{valid_df['sentiment'].value_counts().to_string()}\n\n") # Use to_string for better formatting in file
             else:
                f.write("No valid sentiment classes found.\n\n")
        else:
            f.write("Could not determine total valid samples or class distribution as 'sentiment' column is missing.\n\n")
        ...
        if results and 'mcnemar_results' in results:
            for comparison, mcnemar_data in results['mcnemar_results'].items():
                ...
                if not np.isnan(mcnemar_data['p_value']):
                    ...
                    f.write(f"  {comparison.split('_vs_')[0]} better: {mcnemar_data['model1_better']} cases\n")
                    f.write(f"  {comparison.split('_vs_')[1]} better: {mcnemar_data['model2_better']} cases\n")
                    f.write(f"  Interpretation: {mcnemar_data['interpretation']}\n")
                else:
                    f.write(f"  Interpretation: {mcnemar_data['interpretation']}\n")
        else:
            f.write("McNemar's test could not be performed.\n")


    print("✅ Results saved to 'statistical_analysis_report.txt'")

def main():
    """Main function to execute the complete analysis"""
    print("SENTIMENT ANALYSIS STATISTICAL TESTING")
    print("=" * 50)
    # Removed the file upload explanation prints here as well
    print("Click the 'Choose Files' button below and select all 4 files:")
    print("=" * 50)
    ...
    # Save merged dataset
    try:
        merged_df.to_csv('merged_sentiment_results.csv', index=False, encoding='utf-8-sig')
        print("✅ Merged dataset saved as 'merged_sentiment_results.csv'")
    except Exception as e:
        print(f"⚠️  Error saving merged dataset: {e}")

    # Perform statistical analysis
    print("\nStep 2: Performing statistical analysis...")
    results = perform_statistical_analysis(merged_df)
    # Check if results is None or if required keys are missing
    if results is None:
        print("❌ Statistical analysis failed.")
        results = {} # Initialize an empty dictionary to avoid errors in subsequent steps

    # Create visualizations
    print("\nStep 3: Creating visualizations...")
    # Pass merged_df and results to the visualization function
    create_visualizations(merged_df, results)

    # Save comprehensive report
    print("\nStep 4: Saving comprehensive report...")
    # Pass results and merged_df to the save function
    save_results_to_file(results, merged_df)

    print("\n" + "=" * 50)
    print("ANALYSIS COMPLETED!")
    print("Files generated:")
    print("  - merged_sentiment_results.csv")
    print("  - sentiment_analysis_statistical_comparison.png")
