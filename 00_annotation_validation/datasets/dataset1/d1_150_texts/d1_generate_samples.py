...
# Step 3b: Clean the dataset
# Remove the first column entirely (original col2 becomes col1, original col3 becomes col2)
df = df.drop(df.columns[0], axis=1)

# Map the numeric labels (-1, 0, 1) in the new second column to text labels
label_map = {-1: 'negative', 0: 'neutral', 1: 'positive'}
df[df.columns[1]] = df[df.columns[1]].map(label_map)

# Rename the new first column from 'comment' to 'text'
